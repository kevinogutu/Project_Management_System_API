...
App Structure: Split into accounts, projects, tasks, and reports. Keeps code organized, modular, and scalable.
Custom User: Extends default user with role field,	enables specific permissions (e.g., "Only Managers update status").
Database Models	Project & Task tables	define the core data structure for the ORM to build SQL tables.
Serializers	Python Objects ↔ JSON	APIs speak JSON; serializers handle validation and conversion.
Permissions	Custom Permission Classes	Restrict unauthorized actions (e.g., preventing users from deleting projects).
